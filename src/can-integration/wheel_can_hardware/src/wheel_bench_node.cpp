...
bool status0EverReceived(const spark_max::WheelFeedback & fb)
{
  return fb.status0_stamp != std::chrono::steady_clock::time_point{};
}

bool status0Fresh(const spark_max::WheelFeedback & fb, std::chrono::milliseconds max_age)
{
  if (!status0EverReceived(fb)) {
    return false;
  }
  return (std::chrono::steady_clock::now() - fb.status0_stamp) <= max_age;
}
                      ...
                      const spark_max::WheelFeedback & fb)
{
  ...
  if (!status2_ever && !status0_ever) {
    ss << "  [NO TELEM]";
  } else if ((!status2_ever || !status2_fresh) && (!status0_ever || !status0_fresh)) {
    ss << "  [STALE]";
  } else if (!status2_ever || !status2_fresh) {
    ss << "  [status2 stale]";
  } else if (!status0_ever || !status0_fresh) {
    ss << "  [status0 stale]";
  } else {
    ss << "  [ok]";
  }
...
private:
  ...
  void printTelemetry()
  {
    ...
    for (size_t i = 0; i < device_ids_.size(); ++i) {
      ...
      ss << "\n" << formatRow(
        ...
        status0Fresh(fb, stale_ms_),
        status0EverReceived(fb),
  ...
  void onAnyFrame(uint32_t id, const std::vector<uint8_t> & data)
  {
    ...
    for (uint8_t d : device_ids_) {
      if (d == device_id &&
          (frame_type == spark_max::kStatus0BaseId || frame_type == spark_max::kStatus2BaseId))
      {
        return;
      }
    }
