...
CanControllerNode::CanControllerNode(const rclcpp::NodeOptions& options) :
    ...
    can_send_timer_ = this->create_wall_timer(period, [this] {
        sendCanFrames();
    });
...
void CanControllerNode::getJointStateMessages(const sensor_msgs::msg::JointState::ConstSharedPtr& joint_state_msg) {
    if (joint_state_msg->velocity.size() < 5) {
        logger.error("Received joint state messages with insufficient velocity data. joint size is {}, expected at least 5 (and 7 for servos)",
...
void CanControllerNode::sendCanFrames() {
    ...
    if (send_twist && inhibit_wheel_cmds_) {
        ...
        if (wheel_feedback_&&
            wheel_feedback_
        ->
        getFeedback(static_cast<uint8_t>(DeviceId::ID::WHEEL_MOT1), fb) &&
        ...
        if (wheel_feedback_&&
            wheel_feedback_
        ->
        getFeedback(static_cast<uint8_t>(DeviceId::ID::WHEEL_MOT4), fb) &&
            wheel_feedback_->isStatus2Fresh(
    ...
    if (send_arm && inhibit_arm_cmds_ && joint_state) {
        ...
        if (joint_state->velocity.size() >= 7) {
            ...
            if (arm_active && (spin_deg != 0 || clamp_deg != 0)) {
                RCLCPP_INFO_THROTTLE(
                    this->get_logger(), *this->get_clock(), 1000,
                    "Servo commands sent: spin = %d deg, clamp = %d deg",
                    spin_deg, clamp_deg);
            }
        }
    }
