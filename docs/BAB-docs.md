...
| Field       | Value (idealised) | Meaning            |
|-------------|-------------------|--------------------|
| DevType     | `0x01`            | BAB                |
| Manufacturer| `0x01`            | SCC                |
| DeviceID    | `0x01`            | BAB instance       |

**Observed on the deployed rover** (see `hardware/bab-board/src/battery_board.cpp`): frames use
`DevType=0x00`, `Manufacturer=0x08` (TEAM_USE), `DeviceID=0x00`. Jetson-side parsers must
match the bus, not the idealised table above.

Example observed IDs:

| ID         | DevType | Mfr  | Sev | Instr | DevID | Message  |
|------------|---------|------|-----|-------|-------|----------|
| `0x00088000` | `0x00` | `0x08` | `0x02` | `0x00` | `0x00` | Battery  |
| `0x00088080` | `0x00` | `0x08` | `0x02` | `0x02` | `0x00` | Rail     |
| `0x00088200` | `0x00` | `0x08` | `0x02` | `0x08` | `0x00` | Relay    |
...
Bits 43:42    : Rail index (see table below)
...
### Rail index (deployed rover)

Firmware documentation defaults to `0 = 5V`, `1 = Arm`, `2 = Wheel`. On the deployed rover
those labels are **rotated** (arm→5V, 5V→wheel, wheel→arm):

| Index | PDS channel | Subsystem (rover) | CAN rail command |
|-------|-------------|-------------------|------------------|
| 0     | CH1         | Wheel             | *(none — telemetry only)* |
| 1     | CH2         | 5 V               | `0x000F` |
| 2     | CH3         | Arm               | `0x00F0` |

Command-word rotation (same cycle applied to what each select controls):

| Word     | Firmware doc meaning | Deployed rover meaning |
|----------|----------------------|------------------------|
| `0x000F` | Arm (CH2)            | 5 V rail (CH2)         |
| `0x00F0` | Wheel (CH3)          | Arm rail (CH3)         |
| *(none)* | 5 V (CH1)            | Wheel (CH1, telemetry only) |
...
| Byte 0 (bits 1:0) | Meaning (deployed rover)               |
|--------------------|----------------------------------------|
| `0x00`             | All rails shut down                    |
| `0x01`             | 5 V rail shut down (CH2)               |
| `0x02`             | Arm rail shut down (CH3)               |
...
| Word     | Meaning (deployed rover) |
|----------|--------------------------|
| `0x000F` | 5 V rail (CH2)           |
| `0x00F0` | Arm rail (CH3)           |

CH1 (wheel telemetry) has **no** rail-select command word.
...
msg = can.Message(arbitration_id=ext_id, is_extended_id=True,
                  data=b'\x00\xF0')  # DATA_SELECT arm rail (CH3)
bus.send(msg)
```
